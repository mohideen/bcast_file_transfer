...
directory to one or more servers or S3 buckets. The basic intention is to enable
files uploaded to a single "source" server to be transferred to multiple
"destination" servers or S3 buckets for processing.

## Use Case

The general use case is that an outside entity is periodically (such as once per
day) uploading files to the destination. The uploaded files are typically organized
into subdirectories.

This tool is run periodically to transfer any "new" files to multiple
destinations. A "new" file is any file in the designated
directory on the source server, that is not present in the corresponding
directory of the destination. The directory hierarchy of the files
(starting at the designated directory on the source server) is preserved
when transferring the files to the destination server. For S3 transfers,
the `sub_prefix` option can be configured to create `year` and/or `month` parent
directories on the bucket (expects the filename to include the date).
...
For each "destination", the tool:

1) Compares the directory on the "source" server with the corresponding
directory on destination. Only files/subdirectories missing on the
destination server will be transferred.

1) Transfers the files to the destination.

2) Once all transfers to all destinations is complete, the tool will
send an email indicating the success/failure of the transfers to each of the
destinations.

1) The tool will optionally, on the source server, move successfully
...
the destination. Any files/subdirectories not in the destination
...
* Files can be copied from the source server to the destination server without
  having to enter a password. For server destinations, this is usually done by
  setting up a public/private key on the source server, and adding the public
  key to the destination server. For S3 bucket destinations, this is done by
  by either configuring the credentials in the file `~/.aws/credentials` or by
  setting the AWS credentials environment variables.
...
## SSH Public/Private Key Configuration (For Server Destinations)
...
## AWS Credential Configuration (For S3 Bucket Destinations)

### Option 1: Setting Shared Credentials

```
> vim ~/.aws/credentials
```

Add the credentials of the IAM user with ListObjects and PutObject permission on
...
### Option 2: Setting Credentials Using Environment variables
